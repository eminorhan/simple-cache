"""Testing the effect of cache size. We do separate hyperparameter searches for each cache size.
"""
...
# ---------------------------------------------------------------------------
data_num_range = np.linspace(10000, 100000, 5)  # cache size
repet_range = np.linspace(1, 2, 2)
theta_range = np.linspace(50.0,90.0,5)
lmbd_range = np.linspace(0.3,0.7,5)

FF,RR,TT,LL = np.meshgrid(data_num_range, repet_range, theta_range, lmbd_range)
FF = FF.flatten()
RR = RR.flatten()
TT = TT.flatten()
LL = LL.flatten()

data_frac = FF[job_idx]
repet = RR[job_idx]
theta = TT[job_idx]
lmbd = LL[job_idx]
...
sio.savemat('ResNet%d_cachesize_jobidx%i.mat' %(depth,job_idx), {'mem_acc': test_acc,
                                             'no_mem_acc': scores[1],
                                             'theta': theta,
                                             'lmbd': lmbd})