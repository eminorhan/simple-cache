...
class MultiInputKerasModel(DifferentiableModel):
    ...
    def __init__(
        ...
        images_input = model.input[0] # image is the first input
        ...
        self.aux_lp = aux_lp
        aux_input = model.input[1] #K.placeholder(shape=aux_inp.shape)
        ...
        self._loss_fn = K.function(
            [images_input, label_input, aux_input, aux_learning_phase],
            [loss])
        self._batch_pred_fn = K.function(
            [images_input, aux_input, aux_learning_phase], [predictions])
        self._pred_grad_fn = K.function(
            [images_input, aux_input, aux_learning_phase, label_input],
            [predictions, grad])
        self._bw_grad_fn = K.function(
            [grad_loss_output, images_input, aux_input, aux_learning_phase],
            [grad_loss_input])
    ...
    def batch_predictions(self, images):
        predictions = self._batch_pred_fn([self._process_input(images),self.aux_inp,self.aux_lp])
