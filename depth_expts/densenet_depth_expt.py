"""Depth experiments as in Fig. 2 for DenseNet models
"""
...
model_name = 'densenet40'   # model name: 'densenet40' or 'densenet100'
...
if data_idx==0:
    ...
    if model_name == 'densenet40':
        ...
        available_layers = np.arange(0,157,4)
        available_layers[-1] = 157
        mem_layers = [available_layers[job_idx]] # layers to be used for cache memory
        v = 40
        ...
        available_layers = np.arange(0, 397, 4)
        available_layers[-1] = 397
        mem_layers = [available_layers[job_idx]]  # layers to be used for cache memory
        v = 100
else:
    num_classes = 100
...
sio.savemat('DenseNet_v%d_depthexpt_%i.mat' %(v,job_idx), {'mem_acc': test_acc, 'no_mem_acc': scores[1], 'mem_layer':mem_layers[0]})