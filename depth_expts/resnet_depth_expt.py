"""Depth experiments as in Fig. 2 for ResNet models
"""
...
data_idx = 0                # data index: 0 for CIFAR-10, something else for CIFAR-100
model_name = 'resnet20'     # model name: 'resnet20', 'resnet32' or 'resnet56'

saved_models_dir = '/home/eo41/continuous_cache_image_recognition/saved_models/'  # base directory for saved models
# Set hyperparams: lambda, theta
theta = 80.0
lmbd = 0.5
...
if data_idx==0:
    ...
    if model_name == 'resnet20':
        ...
        available_layers = [0,3,6,10,13,17,20,24,27,32,35,39,42,46,49,54,57,61,64,68,71]
        mem_layers = [available_layers[job_idx]] # layers to be used for cache memory (ResNet20)
        n = 3
    elif model_name == 'resnet32':
        TRAINED_WEIGHTS_FILE = saved_models_dir + 'cifar10_ResNet32v1/cifar10_ResNet32v1_model.150.h5'
        available_layers = [0,3,6,10,13,17,20,24,27,31,34,38,41,46,49,53,56,60,63,67,70,74,77,82,85,89,92,96,99,103,106,110,113]
        mem_layers = [available_layers[job_idx]] # layers to be used for cache memory (ResNet20)
        n = 5
    elif model_name == 'resnet56':
        TRAINED_WEIGHTS_FILE = saved_models_dir + 'cifar10_ResNet56v1/cifar10_ResNet56v1_model.167.h5'
        available_layers = [0, 3, 6, 10, 13, 17, 20, 24, 27, 31, 34, 38, 41, 45, 48, 52, 55, 59, 62, 66, 69, 74, 77, 81,
                            84, 88, 91, 95, 98, 102, 105, 109, 112, 116, 119, 123, 126, 130, 133, 138, 141, 145, 148, 152,
                            155, 159, 162, 166, 169, 173, 176, 180, 183, 187, 190, 194, 197]
        mem_layers = [available_layers[job_idx]]  # layers to be used for cache memory (ResNet20)
        n = 9
else:
    num_classes = 100
    ...
    if model_name == 'resnet20':
        ...
        available_layers = [0,3,6,10,13,17,20,24,27,31,34,38,41,45,48,52,55,59,62,66,69,74,77,81,84,88,91,95,98,102,105,109,112,116,119,123,126,130,
                            133,138,141,145,148,152,155,159,162,166,169,173,176,180,183,187,190,194,197]
        mem_layers = [available_layers[job_idx]] # layers to be used for cache memory (ResNet20)
        n = 9
...
sio.savemat('ResNet%d_depthexpt_%i.mat'%(depth,job_idx), {'mem_acc': test_acc, 'no_mem_acc': scores[1], 'mem_layer':mem_layers[0]})